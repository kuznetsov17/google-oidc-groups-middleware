...
var (
	...
	errConfigMissingAuthorized   = errors.New("authorized.emails, authorized.domains, and/or authorized.groups must be configured")
...
func New(_ context.Context, next http.Handler, config *Config, name string) (http.Handler, error) {
	...
	if len(config.Authorized.Emails) == 0 && len(config.Authorized.Domains) == 0 && len(config.Authorized.Groups) == 0 {
	...
	if len(config.Authorized.Groups) > 0 {
		...
		debug.Printf("Groups authorization enabled for %d groups", len(config.Authorized.Groups))
	}
	...
	authzHandler := &cookieAuthzHandler{
		...
		allowGroups:  toMap(config.Authorized.Groups),
		next:         next,
		authN:        authnHandler,
	}
...
type cookieAuthzHandler struct {
	...
	allowGroups  map[string]struct{} // Allowed Google Groups.
...
func (h *cookieAuthzHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if isAuthorized(ac.Email, ac.Domain, ac.Groups, h.allowEmails, h.allowDomains, h.allowGroups) {
...
type oidcCallbackHandler struct {
	...
	signer        *cookieSigner
	groupsFetcher *groupsFetcher          // nil if groups not configured.
	allowGroups   map[string]struct{}     // Allowed Google Groups.
}
...
func (h *oidcCallbackHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	var groups []string
	if h.groupsFetcher != nil {
		groups, err = h.groupsFetcher.fetchGroups(email)
		if err != nil {
			h.debug.Printf("Failed fetching groups for %s: %v", email, err)
			http.Error(w, "Internal server error", http.StatusInternalServerError)
			return
		}
		if !isGroupMember(groups, h.allowGroups) {
			h.debug.Printf("User %s is not in any allowed group", email)
			http.Error(w, email+" is not authorized", http.StatusUnauthorized)
			return
		}
	}

	if err = h.setCookie(w, email, domain, groups); err != nil {
...
func (h *oidcCallbackHandler) setCookie(w http.ResponseWriter, email, domain string, groups []string) error {
	expires := time.Now().Add(h.config.Cookie.duration)

	ac, err := newAuthCookie(h.signer, expires, email, domain, groups)
	if err != nil {
		return err
	}
...
func newAuthCookie(signer *cookieSigner, expires time.Time, email, domain string, groups []string) (string, error) {
	c := &AuthCookie{
...
// isAuthorized returns true if the email is in allowedEmails, domain is in allowedDomains,
// or any of the user's groups are in allowedGroups (OR semantics).
func isAuthorized(email, domain string, groups []string, allowedEmails, allowedDomains, allowedGroups map[string]struct{}) bool {
	if _, foundEmail := allowedEmails[email]; foundEmail {
		return true
	}

	if _, foundDomain := allowedDomains[domain]; foundDomain {
		return true
	}

	for _, g := range groups {
		if _, foundGroup := allowedGroups[g]; foundGroup {
			return true
		}
	}
