...
func TestCookieAuthzHandler_ServeHTTP(t *testing.T) {
	...
	t.Run("logon_hint is added for expired cookies", func(t *testing.T) {
		...
		v, err := newAuthCookie(h.cookieSigner, time.Now().Add(-1*time.Hour), email, "", nil)
		if err != nil {
			t.Fatal(err)
		}
...
func TestNewAuthCookieFromRequest_MultipleCookies(t *testing.T) {
	...
	t.Run("uses valid cookie when first cookie is expired", func(t *testing.T) {
		...
		expiredValue, _ := newAuthCookie(signer, time.Now().Add(-1*time.Hour), "expired@example.com", "", nil)
		// Create a valid cookie (second in order)
		validValue, _ := newAuthCookie(signer, time.Now().Add(1*time.Hour), "valid@example.com", "", nil)
	...
	t.Run("returns login_hint when all cookies are expired", func(t *testing.T) {
		expiredValue1, _ := newAuthCookie(signer, time.Now().Add(-2*time.Hour), "first@example.com", "", nil)
		expiredValue2, _ := newAuthCookie(signer, time.Now().Add(-1*time.Hour), "second@example.com", "", nil)
